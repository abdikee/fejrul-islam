...
((SELECT id FROM users WHERE email = 'sheikh.abdullah@humsj.edu'), (SELECT id FROM users WHERE email = 'ahmed.hassan@student.edu'), 'Tarbiya Circle Alpha'),
((SELECT id FROM users WHERE email = 'sheikh.abdullah@humsj.edu'), (SELECT id FROM users WHERE email = 'fatima.mohammed@student.edu'), 'Tarbiya Circle Alpha');
...
((SELECT id FROM courses WHERE title = 'Leadership in Islam'), (SELECT id FROM users WHERE email = 'sheikh.abdullah@humsj.edu'), 'Weekly Khutbah Practice', 'Prepare and record a 10-15 minute khutbah on "Youth and Technology in Islam"', 'khutbah', CURRENT_DATE + INTERVAL '7 days', 'high'),
((SELECT id FROM courses WHERE title = 'Quran Recitation Basics'), (SELECT id FROM users WHERE email = 'sheikh.abdullah@humsj.edu'), 'Tajweed Assessment', 'Record recitation of Surah Al-Mulk with proper tajweed rules', 'quran', CURRENT_DATE + INTERVAL '10 days', 'medium');
...
((SELECT id FROM users WHERE email = 'ahmed.hassan@student.edu'), (SELECT id FROM users WHERE email = 'sheikh.abdullah@humsj.edu'), 'khutbah', 'Friday Khutbah: The Importance of Seeking Knowledge', 'Weekly khutbah practice submission', 750, 'reviewed', 85, 'Excellent content and structure. Work on voice modulation and eye contact.'),
((SELECT id FROM users WHERE email = 'ahmed.hassan@student.edu'), (SELECT id FROM users WHERE email = 'sheikh.abdullah@humsj.edu'), 'quran', 'Surah Al-Baqarah (Verses 1-10)', 'Tajweed practice submission', 525, 'pending', NULL, NULL);
...
('New Semester Registration Open', 'Registration for the new semester is now open. Please complete your course selection by the deadline.', 'academic', 'students', 'high', (SELECT id FROM users WHERE email = 'admin@humsj.edu')),
('Ramadan Schedule Changes', 'Please note the adjusted class schedule during the holy month of Ramadan.', 'general', 'all', 'normal', (SELECT id FROM users WHERE email = 'admin@humsj.edu'));
