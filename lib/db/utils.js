...
export async function createUser(userData) {
  ...
  try {
    ...
    const query = `
      INSERT INTO users (email, password_hash, first_name, last_name, gender, department, academic_year, role, phone, email_verified)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, false)
      RETURNING id, email, first_name, last_name, gender, department, academic_year, role, level, phone, email_verified, created_at
    `;

    const values = [email, passwordHash, firstName, lastName, gender, department, academicYear, role, phone || null];
  ...
  try {
    const query = `
      SELECT id, email, password_hash, first_name, last_name, gender, department, 
             academic_year, role, level, profile_photo, bio, is_active, email_verified
      FROM users 
  ...
  try {
    const query = `
      SELECT id, email, first_name, last_name, gender, department, academic_year, 
             role, level, profile_photo, bio, phone, enrollment_date, last_login, email_verified
      FROM users 
