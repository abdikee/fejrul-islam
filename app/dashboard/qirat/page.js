...
import { 
  ...
  History, Scroll, Users, Lightbulb, Languages, ArrowLeft,
  Bell, Settings, Clock
} from 'lucide-react';
...
export default function QiratPage() {
  ...
  const genderColorScheme = user?.gender === 'female' ? {
    primary: 'teal',
    bgGradient: 'from-teal-500 to-purple-500',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50">
      {/* Header */}
      <header className={`${genderColorScheme.headerBg} backdrop-blur-sm border-b ${genderColorScheme.borderColor} sticky top-0 z-40`}>
        <div className="container mx-auto px-4 lg:px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Link
                href={user?.gender === 'female' ? '/dashboard/female' : '/dashboard/male'}
                className={`p-2 ${genderColorScheme.hoverBg} rounded-lg transition-colors`}
                title="Back to Dashboard"
              >
                <ArrowLeft className={`w-5 h-5 ${genderColorScheme.iconColor}`} />
              </Link>
              <div className="w-12 h-12 bg-gradient-to-br from-emerald-500 to-teal-500 rounded-xl flex items-center justify-center">
                <GraduationCap className="w-7 h-7 text-white" />
              </div>
              <div>
                <h1 className="text-xl font-bold text-slate-800">Qirat & Ilm Course System</h1>
                <p className="text-sm text-slate-600">Structured Islamic Education Program</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <button className={`p-2 ${genderColorScheme.hoverBg} rounded-lg transition-colors`}>
                <Bell className={`w-5 h-5 ${genderColorScheme.iconColor}`} />
              </button>
              <button className={`p-2 ${genderColorScheme.hoverBg} rounded-lg transition-colors`}>
                <Settings className={`w-5 h-5 ${genderColorScheme.iconColor}`} />
              </button>
            </div>
          </div>
        </div>
