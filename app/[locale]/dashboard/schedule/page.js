...
import { 
  ...
  BookOpen, Users, Video, MapPin, Bell, Star
} from 'lucide-react';
...
function ScheduleContent() {
  ...
  useEffect(() => {
    // Mock schedule data
    const mockEvents = [
      { id: 1, title: 'Islamic Finance Class', type: 'course', date: new Date(2024, 11, 25, 10, 0), duration: 90, location: 'Room 101', instructor: 'Dr. Ahmad Hassan', color: 'blue' },
      { id: 2, title: 'Quran Study Circle', type: 'study_group', date: new Date(2024, 11, 25, 14, 30), duration: 60, location: 'Masjid', instructor: 'Ustadh Omar', color: 'teal' },
      { id: 3, title: 'Arabic Language Practice', type: 'practice', date: new Date(2024, 11, 26, 9, 0), duration: 45, location: 'Online', instructor: 'Sister Fatima', color: 'green' },
      { id: 4, title: 'Islamic History Lecture', type: 'lecture', date: new Date(2024, 11, 27, 15, 0), duration: 120, location: 'Auditorium', instructor: 'Prof. Ibrahim', color: 'purple' },
      { id: 5, title: 'Fiqh Discussion', type: 'discussion', date: new Date(2024, 11, 28, 11, 0), duration: 75, location: 'Room 205', instructor: 'Sheikh Abdullah', color: 'orange' }
    ];
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setCurrentDate(new Date())} className={`px-4 py-2 text-sm font-medium text-${genderColors.primary}-600 hover:bg-${genderColors.primary}-50 rounded-lg transition-colors`}>
                  Today
                </button>
                <button onClick={() => navigateMonth(1)} className="p-2 hover:bg-slate-100 rounded-lg transition-colors">
            ...
            <div className="grid grid-cols-7 gap-1">
              {days.map((day, index) => {
                ...
                return (
                  ...
                  >
                    <div className={`text-sm font-medium mb-1 ${isToday ? `text-${genderColors.primary}-700` : 'text-slate-800'}`}>
                      {day.getDate()}
                    </div>
                    <div className="space-y-1">
                      {dayEvents.slice(0, 2).map(event => (
                        <div key={event.id} className={`text-xs px-1 py-0.5 bg-${event.color}-100 text-${event.color}-700 rounded truncate`}>
                          {event.title}
                        </div>
                      ))}
                      {dayEvents.length > 2 && (
...
export default function StudentSchedule() {
  return (
    <DashboardPageLayout 
      title="My Schedule" 
      subtitle="Manage your Islamic learning schedule"
      rightContent={
        <button className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-xl font-semibold hover:bg-emerald-700 transition-colors">
